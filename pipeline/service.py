...
def take_next_extra_batch(conn: sqlite3.Connection, session_id: str) -> Optional[Tuple[str, ...]]:
...
def _get_due_follow_up_item_ids(conn: sqlite3.Connection, *, user_id: str, day: date) -> Set[str]:
    rows = conn.execute(
        """
        SELECT item_id
...
def _enqueue_follow_up_queue_item(
    conn: sqlite3.Connection,
    *,
    user_id: str,
    item_id: str,
...
def _enqueue_follow_up_queue_item(
    ...
    *,
    ...
    """Insert a single follow-up queue item (idempotent by user+item+status)."""
    existing = conn.execute(
        "SELECT queue_id FROM follow_up_queue WHERE user_id=? AND item_id=? AND status='pending' LIMIT 1;",
        (user_id, item_id),
    ).fetchone()
    if existing:
    ...
    conn.execute(
        """
        INSERT INTO follow_up_queue(
          queue_id, user_id, item_id, scale_id, reason_code,
          reason_detail_json, priority, earliest_date, status, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?);
        """,
        (
            queue_id,
            ...
            scale_id,
            reason_code,
            json.dumps(reason_detail, ensure_ascii=False) if reason_detail else None,
            float(priority),
            earliest_date.isoformat(),
            now_iso(),
        ),
